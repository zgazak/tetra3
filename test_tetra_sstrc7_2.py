...
def plot_cat(solution, ax):
    ...
    for star in solution["catmatch"]:
        x, y = yf = wf.wcs_world2pix([[np.rad2deg(star[0]), np.rad2deg(star[1])]], 0)[0]
        catx.append(x)
        caty.append(y)
    ...
    quad = [
        wf.wcs_world2pix(np.rad2deg(s[0]), np.rad2deg(s[1]), 0)
        for s in solution["quad"]
    ]

    for idx in range(len(quad) - 1):
        ax.plot(
            [quad[0][0], quad[idx + 1][0]],
            [quad[0][1], quad[idx + 1][1]],
            c="black",
            lw=1,
        )
